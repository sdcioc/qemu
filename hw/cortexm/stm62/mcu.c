        ...
        const STM32Capabilities *capabilities)
{
    ...
    ((STM32USARTState*)usart)->capabilities = capabilities;
    //qdev_prop_set_ptr(DEVICE(usart), "rcc", state->rcc);
    ((STM32USARTState*)usart)->rcc = state->rcc;
    //qdev_prop_set_ptr(DEVICE(usart), "nvic", state->parent_obj.nvic);
    ((STM32USARTState*)usart)->nvic = state->parent_obj.nvic;
